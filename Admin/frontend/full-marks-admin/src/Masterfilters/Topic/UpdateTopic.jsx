...
const UpdateTopic = () => {
  ...
  const [topicData, setTopicData] = useState({ topicName: '', sortOrder: '' });
  ...
  const fetchTopicData = async () => {
    try {
      ...
      if (data.success) {
        setTopicData({ topicName: data.class.class_name, sortOrder: data.class.sort_order });
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost/fullmarks-server/Masterfilter/Classes/updateclasses.php', {
        ...
        body: JSON.stringify({ class_id: id, ...topicData }),
      });
      const data = await response.json();
      if (data.success) {
        ...
        <div className="row">
          ...
          <div className="col-md-9">
            <div className="container mt-3">
              {/* Topbar */}
              <h5 className='text-grey my-3'>Update Topic</h5>
              <div className="row"></div>
    <div className="container mt-3 bg-white shadow-lg p-3 mb-5 bg-white rounded">
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label htmlFor="className" className= 'mb-2'><b>Topic</b></label>
          <input
            type="text"
